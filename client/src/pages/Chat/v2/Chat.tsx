...
export default function Chat() {
  ...
  const checkMessage = (key: any): void => {
    if (key.keyCode === 13 && isDesktop() && !key.shiftKey) {
      if (!isEmpty(message)) key.preventDefault();
      sendMessage();
    }
  };
  ...
  return (
    <>
      <div
        className="grid grid-rows-[1fr,auto] gap-[5px] h-[100%]"
        ref={scrollRef}
      >
        <div
          className="p-[0.9rem_1.1rem] pr-[.7rem] relative bottom-[0] pb-[.5rem] grid items-end"
          style={{ overflow: messagesLoaded ? "scroll" : "hidden" }}
        >
          <div
            id="messages"
            className="items-end grid p-[.15rem_.25rem] pr-[.3rem]"
            ref={messagesRef}
        ...
        <div className="relative bottom-[0] grid items-end">
          <div className="bg-[transparent] p-[1.1rem_0]  pt-[0rem] grid grid-rows-[1fr,auto] gap-[0px]">
            ...
            >
              <b>{typingText.user}</b> {typingText.users > 0 ? " and " : null}{" "}
              {typingText.users > 0 ? <b>{typingText.users}</b> : null}{" "}
              {typingText.content}
            </p>
            <div
              className={`${
