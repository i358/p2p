...
export default class connectionControl {
  ...
  readTest(): Promise<any> {
    return new Promise(async (resolve, reject) => {
      ...
      await redis.set("dataReadTest", "Redis").catch((err) => reject(err));
      let isExists = await redis
        .exists("dataReadTest")
  ...
  writeTest(): Promise<any> {
    return new Promise(async (resolve, reject) => {
      let redis = this.redis;
      const now = Date.now();
      redis.set("last_acknowledged", now).catch((err) => reject(err));
      const redisNow: unknown = await redis
