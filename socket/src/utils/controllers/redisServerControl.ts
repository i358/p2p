...
let redisConnectionURI: string = `redis://${process.env.REDIS_PASSWORD ? ":" + process.env.REDIS_PASSWORD : ""}@${process.env.REDIS_HOST || "localhost"}:${process.env.REDIS_PORT || 6379}`
...
const initalize = async (): Promise<boolean> => {
  ...
  await redis.connect()
  .catch((err) => { return log("{error} An error occurred during connecting to the Redis server -> " + err, colors.red) })
  ...
  await cc.readTest()
    .catch(() => { return log("{error} An error occurred during reading data from Redis cluster.", colors.red) })
...
await cc.writeTest()
    .catch(() => { return log("{error} An error occurred during writing data to Redis cluster.", colors.red) })
