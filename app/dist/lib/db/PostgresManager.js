...
(function (PostgresManager) {
    ...
    class Postgres {
        ...
        Connect() {
            return new Promise((resolve, reject) => {
                const { POSTGRES_DBNAME, POSTGRES_HOST, POSTGRES_PASSWORD, POSTGRES_USERNAME, MODE, } = process.env;
        ...
        Find(options) {
            ...
            return new Promise((resolve, reject) => {
                if (!table)
                    throw "Query keys and values or table name??";
                let select = pick ? pick.map((p) => `"${p}"`).join(",") : "*";
                let where = keys && values
                    ? `WHERE ${keys
                        .map((key, id) => `"${key}"=$${id + 1}`)
                        .join(" AND ")}`
                    : "";
                Client.query(`SELECT ${select} FROM "${table}" ${where}`, values || [], (err, res) => {
                    if (err)
                        reject(err);
                    if (sort) {
        ...
        Update(options) {
            ...
            return new Promise((resolve, reject) => {
                let where = keys && values
                    ...
                    : "";
                let set = `${newKeys
                    .map((newK, id) => `"${newK}"=$${id + 1}`)
                    .join(", ")}`;
                let queryString = `UPDATE "${table}" SET ${set} ${where}`;
                Client.query(queryString, newValues, (err, r) => {
                    if (err)
                        reject(err);
                    resolve("ok.");
                });
    ...
    PostgresManager.Postgres = Postgres;
})(PostgresManager || (exports.PostgresManager = PostgresManager = {}));
//# sourceMappingURL=PostgresManager.js.map